...
function handleResult(resultData) {
    ...
    pageTitleElement.append(resultData.movieTitle);

    let movieTitleElement = jQuery("#movie_title");
    movieTitleElement.append(resultData.movieTitle);

    let movieInfoElement = jQuery("#movie_details");

    rowHTML = "";
    rowHTML += "<p><strong>Year Released:</strong>" + " " + resultData.movieYear + "</p>";
    rowHTML += "<p><strong>Director:</strong>" + " " + resultData.movieDirector + "</p>";
    rowHTML += "<p><strong>Rating:</strong>" + " " + resultData.movieRating + "</p>";

    let genreList = resultData.movieGenres.split(',');
    let genres = "";
    for (let i = 0; i < genreList.length; i++) {
        genres += '<a href="movie-list.html?action=browseGenre&value=' + genreList[i] + '">';
        genres += genreList[i] + '</a>,';
    }
    rowHTML += "<p><strong>Genres:</strong>" + " " + genres.split(",").join(", ").trim().slice(0, -1) + "</p>";

    let starsIdList = resultData.movieStarsId.split(',');
    let starsList = resultData.movieStars.split(',');
    let stars = "";
    for (let i = 0; i < starsIdList.length; i++) {
        stars += '<a href="single-star.html?id=' + starsIdList[i] + '">' + starsList[i] + '</a>,';
    }
    rowHTML += "<p><strong>Stars:</strong>" + " " + stars.split(",").join(", ").trim().slice(0, -1) + "</p>";


    rowHTML += '<div class="add-to-cart-button" data-movie-id="' + resultData.movieId + '">' +
        '<form method="post">' +
        '<input name="action" type="hidden" id="add-cart" value="add">' +
        '<input name="movieId" type="hidden" value="' + resultData.movieId + '">' +
        '<input name="movieTitle" type="hidden" value="' + resultData.movieTitle + '">' +
...
jQuery.ajax({
    ...
    url: "api/single-movie?id=" + getParameterByName('id'),
