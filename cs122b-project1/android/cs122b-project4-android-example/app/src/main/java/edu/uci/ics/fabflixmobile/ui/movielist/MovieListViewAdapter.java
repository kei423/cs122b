...
public class MovieListViewAdapter extends ArrayAdapter<Movie> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String genresString = Arrays.toString(ArrayUtils.subarray(movie.getGenres(), 0, 3));
        String starsString = Arrays.toString(ArrayUtils.subarray(movie.getStars(), 0, 3));
        viewHolder.subtitle.setText("Rating: " + movie.getRating() +
                "\nDirector: " + movie.getDirector() + "\nGenres: "
                + genresString.substring(1, genresString.length()-1) + "\nStars: "
                + starsString.substring(1, starsString.length()-1));
